...
class LegoGrid {
    constructor() {
        ...
        this.blockModels = [];
        this.placeBlock = (block, position, rotation, speed) => {
            ...
            for (let i = 0; i != rotatedGridModel.length; i += 1) {
                if (rotatedGridModel[i].column == -0) {
                    rotatedGridModel[i].column = 0;
                }
                if (rotatedGridModel[i].row == -0) {
                    rotatedGridModel[i].row = 0;
                }
            }
            ...
            for (let vector of rotatedGridModel) {
                ...
                try {
                    if (this.data[layerPos][rowPos][columnPos] == undefined) {
                        throw ("Triggering try-catch");
                    } //just accessing it to trigger the try-catch
                }
                catch (_a) {
